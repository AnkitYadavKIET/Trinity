...
def main():
    ...
    while True:
        # Get stock symbol
        symbol = input("\n🏷️  Enter stock symbol (or 'done' to finish): ").strip().upper()
        
        if symbol.lower() == 'done':
            if len(orders_data) == 0:
                print("⚠️  You must add at least one order!")
                continue
            break
        
        # Validate symbol format
        if not symbol.startswith("NSE:") or not symbol.endswith("-EQ"):
            print("⚠️  Invalid format! Use NSE:SYMBOL-EQ format (e.g., NSE:SBIN-EQ)")
        ...
        # Get quantity
        while True:
            qty_input = input(f"📦 Enter quantity for {symbol}: ").strip()
            try:
                qty = int(qty_input)
                if qty <= 0:
        ...
        print(f"✅ Added: SELL {qty} x {symbol} @ MARKET")
